...
Content based filtering (only based on the current users past data)
Collaborative Filtering (finding similar users & finding similar products)
...
Multi layer perceptron model - A deep neural network is used to learn the non-linear function rather than a linear one and in doing so utilize the expressiveness of the final model which will take into account complex relationship between datapoints
The Matrix Factorization network is used which is a regular point-wise matrix factorization to approximate the factorization of a large and sparse (user x item) matrix into the two lower dimensional matrices (user x features) and (features x items) using embedding layer.
Combine both models to utilize the strength of both models to make better predictions
Improvement
